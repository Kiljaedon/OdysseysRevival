...
var _active_effects_by_name: Dictionary = {}  # {name_id: GameplayEffect} for O(1) lookup
...
func _process(delta: float) -> void:
	...
	for e2 in to_remove:
		...
		_active_effects_by_name.erase(e2.name_id)  # Also remove from dictionary
...
func add_effect(eff: GameplayEffect, source: AbilitySystemComponent = null) -> void:
	# Stacking/refresh - O(1) lookup using dictionary
	if _active_effects_by_name.has(eff.name_id):
		var existing = _active_effects_by_name[eff.name_id]
	...
	# Add new effect (maintain both array and dictionary)
	_active_effects.append(eff)
	_active_effects_by_name[eff.name_id] = eff
	eff._source = source
	if eff.duration > 0.0:
...
func remove_effect_by_name(id: StringName) -> bool:
	...
	if not _active_effects_by_name.has(id):
		return false

	var effect = _active_effects_by_name[id]
	effect.on_removed(self)

	# Remove from both array and dictionary
	_active_effects.erase(effect)
	_active_effects_by_name.erase(id)
	return true

func dispel(mask: int, max_count: int = 999, only_debuffs: bool = true, tag_filter: StringName = StringName("")) -> int:
	var removed: int = 0
	var to_remove: Array = []  # Collect effects to remove

	for ge: GameplayEffect in _active_effects:
	...
	# Remove collected effects (avoids duplicate() and multiple erases)
	for ge in to_remove:
		ge.on_removed(self)
		_active_effects.erase(ge)
		_active_effects_by_name.erase(ge.name_id)  # Also remove from dictionary
		removed += 1
