...
func on_client_delta(bytes: PackedByteArray) -> void:
	...
	# Validate pairs against whitelist of writable fields.
	var validated_pairs: Array = []
	for p in pairs:
		if p.size() < 2:
			continue
		var fid: int = p[0]
		if _is_client_owned(fid):
			validated_pairs.append(p)
		else:
			printerr("Security Warning: Peer %d tried to modify unauthorized field ID %d" % [sender, fid])

	var syn: StateSynchronizer = entities.get(eid, null)
	if syn != null and validated_pairs.size() > 0:
		syn.apply_delta(validated_pairs)
		syn.mark_many_by_id(validated_pairs, false)
		_track_client_pairs(eid, validated_pairs)
