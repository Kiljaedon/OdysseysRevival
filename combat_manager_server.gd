...
func handle_npc_attack_request(peer_id: int, npc_id: int):
	...
	# Generate enemy squad: attacked NPC (boss) + 5 random other NPCs
	var enemy_squad = []
	...
	if not boss_data.is_empty():
		...
		# Use actual stats from derived_stats
		if boss_data.has("derived_stats"):
			boss_data["max_hp"] = boss_data.derived_stats.get("max_hp", 100)
			boss_data["hp"] = boss_data["max_hp"]  # Boss starts at full HP
			boss_data["max_mp"] = boss_data.derived_stats.get("max_mp", 50)
			boss_data["mp"] = boss_data["max_mp"]
			boss_data["max_energy"] = boss_data.derived_stats.get("max_ep", 100)
			boss_data["energy"] = boss_data["max_energy"]
			boss_data["attack"] = boss_data.derived_stats.get("phys_dmg", 10)
			boss_data["defense"] = boss_data.derived_stats.get("phys_def", 10)
		else:
			boss_data["max_hp"] = 100
			boss_data["hp"] = 100
			boss_data["max_mp"] = 50
			boss_data["mp"] = 50
			boss_data["max_energy"] = 100
			boss_data["energy"] = 100
			boss_data["attack"] = 10
			boss_data["defense"] = 10
	...
	npc_types.shuffle()  # Randomize order
	...
	for i in range(5):
		...
		if not enemy_data.is_empty():
			...
			# Use actual stats from derived_stats
			if enemy_data.has("derived_stats"):
				enemy_data["max_hp"] = enemy_data.derived_stats.get("max_hp", 100)
				enemy_data["hp"] = enemy_data["max_hp"]  # Always start at full HP
				enemy_data["max_mp"] = enemy_data.derived_stats.get("max_mp", 50)
				enemy_data["mp"] = enemy_data["max_mp"]
				enemy_data["max_energy"] = enemy_data.derived_stats.get("max_ep", 100)
				enemy_data["energy"] = enemy_data["max_energy"]
				enemy_data["attack"] = enemy_data.derived_stats.get("phys_dmg", 10)
				enemy_data["defense"] = enemy_data.derived_stats.get("phys_def", 10)
			else:
				enemy_data["max_hp"] = 100
				enemy_data["hp"] = 100  # Always start at full HP
				enemy_data["max_mp"] = 50
				enemy_data["mp"] = 50
				enemy_data["max_energy"] = 100
				enemy_data["energy"] = 100
				enemy_data["attack"] = 10
				enemy_data["defense"] = 10
