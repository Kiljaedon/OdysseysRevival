...
func _ready():
	...
	# Create and load default test map
	create_default_test_map()

	# Load first TMX map if available
	if available_maps.size() > 1:
		...
		if map_dropdown and available_maps.size() > 0:
			...
			for map_name in available_maps:
				if map_name == "default_test":
					map_dropdown.add_item("Default Test Map")
				else:
...
func load_selected_map(map_name: String):
	...
	if map_manager:
		...
		# For TMX maps, we need to load collision objects
		# MapManager doesn't have direct access to CollisionSystemManager,
		# so we handle collision loading here after map loads
		if map_name != "default_test":
			# Load collision objects from the TMX file
			...
			if FileAccess.file_exists(tmx_path):
				...
				if file:
					...
					# Extract collision objectgroups
					...
					for objectgroup_result in objectgroup_results:
						...
						if "collision" in group_name.to_lower():
							if collision_system_manager:
								# Use default 32x32 tile size
								collision_objects_loaded += collision_system_manager.load_collision_objects(group_content, 32, 32)

					print("[DevClient] Loaded ", collision_objects_loaded, " collision objects from TMX")
	else:
		print("WARNING: Map manager not initialized")

func create_default_test_map():
	"""Delegation wrapper for MapManager.create_default_test_map()"""
	if map_manager:
		if map_info:
			map_manager.set_map_info_label(map_info)
		if collision_system_manager:
...
func create_game_screen_content(parent: Panel):
	...
	if world_node:
		world_node.get_parent().remove_child(world_node)
		subviewport.add_child(world_node)

		# Update node references to point to SubViewport location
		# GameWorld is now direct child of SubViewport
		bottom_layer = world_node.get_node("BottomLayer")
		middle_layer = world_node.get_node("MiddleLayer")
		top_layer = world_node.get_node("TopLayer")
		test_character = world_node.get_node("TestCharacter")
		animated_sprite = world_node.get_node("TestCharacter/AnimatedSprite2D")
		camera = world_node.get_node("TestCharacter/Camera2D")

func create_map_selector_content(parent: Panel):
...
func create_character_tester_content(parent: Panel):
	var vbox = VBoxContainer.new()
	vbox.position = Vector2(10, 40)
	...
	direction_dropdown.item_selected.connect(_on_direction_selected)
...
func create_dev_tools_content(parent: Panel):
	var vbox = VBoxContainer.new()
	vbox.position = Vector2(10, 40)
...
func create_controls_reference_content(parent: Panel):
	var vbox = VBoxContainer.new()
	vbox.position = Vector2(10, 40)
...
func _on_panel_locked(panel):
	print("Panel locked: ", panel.panel_title)

func _on_panel_unlocked(panel):
	print("Panel unlocked: ", panel.panel_title)

# NOTE: UI layout functions removed - call ui_layout_manager directly

func create_sprite_loader_content(parent: Panel):
	var vbox = VBoxContainer.new()
	vbox.position = Vector2(10, 40)
...
func _on_load_character_from_dropdown(dropdown: OptionButton):
	var selected_index = dropdown.selected
	if selected_index > 0 and character_sprite_manager:  # Skip "Select Character..." option
		var character_name = dropdown.get_item_text(selected_index)
		var data = character_sprite_manager.load_character_data(character_name)
		character_data_cache = character_sprite_manager.character_data_cache
		# Also update the main character tester dropdown
		if class_dropdown:
			for i in range(class_dropdown.get_item_count()):
				if class_dropdown.get_item_text(i) == character_name:
